...
function Test-AutorunsStartup {
    ...
    try {
        ...
        $wmiFilters = $null
        $wmiConsumers = $null
        $wmiActiveScript = $null
        try {
            $wmiFilters = Get-WmiObject -Namespace "root\subscription" -Class __EventFilter -ErrorAction Stop
        } catch {
            if ($_.Exception.Message -match 'Access denied|Access is denied|0x80041003') {
                $wmiAccessOK = $false
            }
        }

        if ($wmiAccessOK) {
            try { $wmiConsumers = Get-WmiObject -Namespace "root\subscription" -Class CommandLineEventConsumer -ErrorAction SilentlyContinue } catch {}
            try { $wmiActiveScript = Get-WmiObject -Namespace "root\subscription" -Class ActiveScriptEventConsumer -ErrorAction SilentlyContinue } catch {}
            ...
            Write-DatSub "Cannot access WMI subscriptions (requires admin)"
        }
